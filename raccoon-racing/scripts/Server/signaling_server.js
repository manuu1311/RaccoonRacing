...
const MAX_LOBBY_PLAYERS = 4;   // host + up to 3 clients
...
const STR_LOBBY_IS_FULL = "Lobby is full";
...
class Lobby {
    ...
    leave (peer) {
        ...
        // FIX: exclude the leaving peer itself from notifications below —
        // its socket is already closing/closed at this point (called from
        // the ws "close" handler), so messaging it either no-ops or throws.
        const remaining = this.peers.filter((p) => p !== peer);
        remaining.forEach((p) => {
...
function joinLobby (peer, pLobby) {
    // FIX: this check used to only run when creating a NEW lobby
    // (pLobby === ""), so a peer already in lobby A could send another
    // "J: <codeB>" and get silently double-joined into lobby B, orphaning
    // itself in lobby A forever. Now it always applies.
    if (peer.lobby !== "") {
    ...
    if (lobby.peers.length >= MAX_LOBBY_PLAYERS) {
        throw new ProtoError(4000, STR_LOBBY_IS_FULL);
    }
